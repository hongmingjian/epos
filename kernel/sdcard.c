...
uint64_t timer_getTickCount (void)
{
	...
	do {
		resVal = pst->chi; 								// Read Arm system timer high count
		lowCount = pst->clo;							// Read Arm system timer low count
	} while (resVal != (uint64_t)pst->chi);				// Check hi counter hasn't rolled in that time
...
****************************************************************************/
#define EMMC_ARG2			((volatile __attribute__((aligned(4))) uint32_t*)(uintptr_t)(MMIO_BASE_VA + 0x300000))
#define EMMC_BLKSIZECNT		((volatile struct __attribute__((aligned(4))) regBLKSIZECNT*)(uintptr_t)(MMIO_BASE_VA + 0x300004))
#define EMMC_ARG1			((volatile __attribute__((aligned(4))) uint32_t*)(uintptr_t)(MMIO_BASE_VA + 0x300008))
#define EMMC_CMDTM			((volatile struct __attribute__((aligned(4))) regCMDTM*)(uintptr_t)(MMIO_BASE_VA + 0x30000c))
#define EMMC_RESP0			((volatile __attribute__((aligned(4))) uint32_t*)(uintptr_t)(MMIO_BASE_VA + 0x300010))
#define EMMC_RESP1			((volatile __attribute__((aligned(4))) uint32_t*)(uintptr_t)(MMIO_BASE_VA + 0x300014))
#define EMMC_RESP2			((volatile __attribute__((aligned(4))) uint32_t*)(uintptr_t)(MMIO_BASE_VA + 0x300018))
#define EMMC_RESP3			((volatile __attribute__((aligned(4))) uint32_t*)(uintptr_t)(MMIO_BASE_VA + 0x30001C))
#define EMMC_DATA			((volatile __attribute__((aligned(4))) uint32_t*)(uintptr_t)(MMIO_BASE_VA + 0x300020))
#define EMMC_STATUS			((volatile struct __attribute__((aligned(4))) regSTATUS*)(uintptr_t)(MMIO_BASE_VA + 0x300024))
#define EMMC_CONTROL0		((volatile struct __attribute__((aligned(4))) regCONTROL0*)(uintptr_t)(MMIO_BASE_VA + 0x300028))
#define EMMC_CONTROL1		((volatile struct __attribute__((aligned(4))) regCONTROL1*)(uintptr_t)(MMIO_BASE_VA + 0x30002C))
#define EMMC_INTERRUPT		((volatile struct __attribute__((aligned(4))) regINTERRUPT*)(uintptr_t)(MMIO_BASE_VA + 0x300030))
#define EMMC_IRPT_MASK		((volatile struct __attribute__((aligned(4))) regIRPT_MASK*)(uintptr_t)(MMIO_BASE_VA + 0x300034))
#define EMMC_IRPT_EN		((volatile struct __attribute__((aligned(4))) regIRPT_EN*)(uintptr_t)(MMIO_BASE_VA + 0x300038))
#define EMMC_CONTROL2		((volatile struct __attribute__((aligned(4))) regCONTROL2*)(uintptr_t)(MMIO_BASE_VA + 0x30003C))
#define EMMC_TUNE_STEP 		((volatile struct __attribute__((aligned(4))) regTUNE_STEP*)(uintptr_t)(MMIO_BASE_VA + 0x300088))
#define EMMC_SLOTISR_VER	((volatile struct __attribute__((aligned(4))) regSLOTISR_VER*)(uintptr_t)(MMIO_BASE_VA + 0x3000fC))
...
. 10Aug17 LdB
.--------------------------------------------------------------------------*/
static uint8_t fls_uint32_t (uint32_t x)
{
	uint8_t r = 32;											// Start at 32
...
static uint32_t sdGetClockDivider (uint32_t freq)
{
	...
	if (EMMC_SLOTISR_VER->SDVERSION < 2) {							// Any version less than HOST SPECIFICATION 3 (Aka numeric 2)
		uint8_t shiftcount = fls_uint32_t(divisor);			// Only 8 bits and set pwr2 div on Hosts specs 1 & 2
...
static SDRESULT sdSetClock (uint32_t freq)
{
	...
	uint32_t cdiv = sdGetClockDivider(freq);					// Fetch divisor for new frequency
...
. 21Aug17 LdB
.--------------------------------------------------------------------------*/
SDRESULT sdTransferBlocks (uint32_t startBlock, uint32_t numBlocks, uint8_t* buffer, int write )
{
			...
			while (priv->bPos <  512) {								// While not a buffer end
				...
				if (dir->name[0] != FILE_DELETED) {					// If entry not deleted
					...
					} else if (dir->attrib == FILE_ATTRIBUTE_LONGNAME)// LFN block precede the normal block
					{
						...
						uint32_t dotadded = 0;					// Track if we add a dot .. it starts zero
...
static int sd_read(struct dev *dp, uint32_t addr, uint8_t *buf, size_t buf_size)
{
	...
	rem = addr & (512-1);
	if(rem) {
		ret = sdTransferBlocks((addr-rem)/512, 1, tmpbuf, 0);
		if(ret != SD_OK)
			return buf-oldbuf;
	...
	rem = buf_size & (~(512-1));
	if(rem) {
		ret = sdTransferBlocks( addr/512, buf_size/512, buf, 0 );
		if(ret != SD_OK)
			return buf-oldbuf;
	...
	if(buf_size) {
		ret = sdTransferBlocks( addr/512, 1, tmpbuf, 0 );
		if(ret != SD_OK)
			return buf-oldbuf;
...
static int sd_write(struct dev *dp, uint32_t addr, uint8_t *buf, size_t buf_size)
{
	...
	if(rem) {
		...
		ret = sdTransferBlocks( (addr-rem)/512, 1, tmpbuf, 1 );
		if(ret != SD_OK)
			return buf-oldbuf;
	...
	rem = buf_size & (~(512-1));
	if(rem) {
		ret = sdTransferBlocks( addr/512, buf_size/512, buf, 1 );
		if(ret != SD_OK)
			return buf-oldbuf;
	...
	if(buf_size) {
		...
		ret = sdTransferBlocks( addr/512, 1, tmpbuf, 1 );
		if(ret != SD_OK)
			return buf-oldbuf;
