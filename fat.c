...
int  fat_fseek(int fd, unsigned int offset, int whence)
{
	...
	switch(whence) {
		case SEEK_SET:
			...
			/* 计算偏移量 */
			cluster_index = offset / (g_fat_struct.BytsPerSect * g_fat_struct.SectsPerClus);
			sector_index  = (offset / g_fat_struct.BytsPerSect) % g_fat_struct.SectsPerClus;
			sector_offset = offset % g_fat_struct.BytsPerSect;
		...
		case SEEK_CUR:
			...
			free_bytes_per_cluster = (g_fat_struct.SectsPerClus - 1 - sector_index) * g_fat_struct.BytsPerSect +
				(g_fat_struct.BytsPerSect - fp->SectOffset);
			if(offset <= free_bytes_per_cluster) {
				/* 定位的文件位置属于当前簇 */
				if(offset <= (g_fat_struct.BytsPerSect - fp->SectOffset)) {		/* 定位的文件在当前扇区内 */
					sector_index = 0;
					sector_offset = fp->SectOffset + offset;
				} else {	/* 定位的文件超出当前扇区 */
					sector_index  = (offset - (g_fat_struct.BytsPerSect - fp->SectOffset)) / g_fat_struct.BytsPerSect + 1;
					sector_offset = (offset - (g_fat_struct.BytsPerSect - fp->SectOffset)) % g_fat_struct.BytsPerSect;
				}
				...
				/* 计算偏移量 */
				cluster_index = (offset - free_bytes_per_cluster) / (g_fat_struct.BytsPerSect * g_fat_struct.SectsPerClus) + 1;
				sector_index  = ((offset - free_bytes_per_cluster) / g_fat_struct.BytsPerSect) % g_fat_struct.SectsPerClus;
				sector_offset = (offset - free_bytes_per_cluster) % g_fat_struct.BytsPerSect;
				cluster = SectNum2ClusNum(fp->CurrentSect);
